...
const HistogramChart = ({ threshold, data, onChangeThreshold = () => {} }) => {
  ...
  if (data) {
    data.bucketMeans.forEach((value, i) => output.push({ mean: value, amount: data.histogram[i] }))
  }

  return (
    <div>
      <BarChart data={output} width={ChartWidth} height={200}
      ...
      >
        ...
        {
          threshold && data &&
          <ReferenceLine x={threshold} alwaysShow isFront={true} stroke='#EF0000' />
        }
      </BarChart>
    </div>
  )
