...
export const Actions = {
  addEELayer: (overlay, identifier, position) => ({
    type: Types.ADD_EE_LAYER,
    payload: { overlay, identifier, position },
  }),
  addEEFeature: (feature, name, color = "#FF0000", opacity = 1, group) => ({
    type: Types.ADD_EE_FEATURE,
  ...
  completeDrawing: (overlay, coordinates) => ({
    type: Types.COMPLETE_DRAWING,
    payload: { overlay, coordinates },
  }),
  requestDrawing: (drawingType, message) => ({
    type: Types.REQUEST_DRAWING,
    payload: { drawingType, message },
  }),
  addShapes: (shapes, name, group = "all", content = null) => ({
    type: Types.ADD_SHAPES,
    payload: { shapes, name, group, content },
  }),
  highlight: (index) => ({
  ...
  removeShapeGroup: (group) => ({
    type: Types.REMOVE_SHAPE_GROUP,
    payload: { group },
  }),
  centralizeMap: (coordinates) => ({
    type: Types.CENTRALIZE_MAP,
    payload: { coordinates },
  }),
};
