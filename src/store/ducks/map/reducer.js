  ...
  switch (type) {
    ...
    case Types.ADD_SHAPES: {
      const shapes = update(state.shapes, {
        $push: [{ overlays: payload.shapes, name: payload.name, group: payload.group, content: payload.content }]
      })
    ...
    case Types.REQUEST_DRAWING: {
      return { ...state, currentlyDrawing: true, drawingMessage: payload.message }
    }
