...
export const calculateDistances = (transect, baseline, shorelines) => {
  ...
  const distances = shorelines.map((shoreline) => {
    ...
    const distanceReference = ee
      .FeatureCollection(intersections.map(distanceTo))
      .limit(1, distanceKey);
  ...
  return ee.Dictionary(
    ee.Algorithms.If(distances.size().gt(0), returnKeyring(distances), {})
  );
};
...
export const estevesLabelling = (lrr) => {
  const classification = ee.Algorithms.If(
    lrr.lt(-1.0),
...
export const calculateStatistics = (measurement) => {
  ...
  const guess = ee.Dictionary(
    ee.Algorithms.If(count.gt(0), distanceList.get(0), {
      date: ee.Date(0),
      distance: ee.Number(0),
    })
  );
  ...
  const stats = ee.Dictionary(
    distanceList.slice(1).iterate((item, acc) => {
      ...
      const earliest = ee.Algorithms.If(
        ee
          .Date(current.get("date"))
          .millis()
          .lt(ee.Date(minByDate.get("date")).millis()),
        current,
        minByDate
      );

      const latest = ee.Algorithms.If(
        ee
          .Date(current.get("date"))
          .millis()
          .gt(ee.Date(maxByDate.get("date")).millis()),
        current,
        maxByDate
      );
      ...
      const closest = ee.Algorithms.If(
        current.getNumber("distance").lt(minByDistance.getNumber("distance")),
        current,
        minByDistance
      );

      const farthest = ee.Algorithms.If(
        current.getNumber("distance").gt(maxByDistance.getNumber("distance")),
        current,
        maxByDistance
      );
  ...
  const sce = farthest
    .getNumber("distance")
    .subtract(closest.getNumber("distance"));

  // NSM
  const nsm = latest
    .getNumber("distance")
    .subtract(earliest.getNumber("distance"));

  // EPR
  const epr = nsm.divide(
    ee.Date(latest.get("date")).difference(earliest.get("date"), "year")
  );
...
export const complementaryProperties = (transect, measurement, keepProps) => {
  ...
  const distances = serializeList(
    distanceInfo.map((item) => ee.Dictionary(item).getNumber("distance"))
  );
 ...
 */
export const generateTransectsStatistics = (
  transects,
  baseline,
  shorelines,
  keepProps
  ...
  return transects.map((input) => {
    ...
    return transect.setMulti(
      extra.combine(stats).combine({ [INTERNALS]: internals })
    );
  });
};
