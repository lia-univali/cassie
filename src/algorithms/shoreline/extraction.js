 ...
 */
export const identifyWaterFeature = (
  image,
  geometry,
  scale,
  bands,
...
export const removeShorelineNoise = (shorelines, transects) => {
  ...
  return ee
    .Feature(ee.FeatureCollection(weighted).limit(1, "weight", false).first())
    .geometry();
};
...
export const gaussianKernel = (size, mean, sigma) => {
  const gaussianCurve = (x, mean, sigma) => {
    ...
    const exponent = ee.Number(-1).multiply(
      ee
        .Number(x)
        .subtract(mean)
        .pow(2)
  ...
  // in the interval
  const kernel = ee.List.sequence(begin, end).map((i) =>
    gaussianCurve(i, mean, sigma)
  );
 ...
 */
export const linearGaussianFilter = (
  coordinates,
  samples = 3,
  mean = 0,
  sd = 0.75
  ...
  const sequence = ee.List.sequence(
    ee.Number(0),
  ...
  const path = sequence.map((index) => {
    ...
    const smoothenLong = gaussian
      .map((point) => ee.List(point).get(0))
      .reduce(ee.Reducer.sum());
    const smoothenLat = gaussian
      .map((point) => ee.List(point).get(1))
      .reduce(ee.Reducer.sum());
...
export const thresholdingAlgorithm = (histogram, count) => {
  ...
  const bss = ee.List.sequence(1, size).map((i) => {
    ...
    return aCount
      .multiply(aMean.subtract(mean).pow(2))
...
export const extractShoreline = (
  ...
  const waterSegment = identifyWaterFeature(
    image,
    geometry,
    scale,
    bands,
  ...
  return ee.Geometry.MultiLineString(waterSegment.coordinates()).intersection(
    geometry.buffer(-10)
  );
};
...
export const deriveShoreline = (
  ...
  const shoreline = extractShoreline(
    image,
    geometry,
    scale,
    bands,
...
export const deriveShorelines = (
  ...
  const data = dates.map((value) => {
    const mission = satellite.missions.find(
      (mission) => mission.name === value.name
    );
